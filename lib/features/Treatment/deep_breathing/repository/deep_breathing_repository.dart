...
class TreatmentRepository {
  ...
  Future<String> trackUserTreatment({
    ...
    double progress = 0.0,
    String?
        userTreatmentId, // Optional parameter to support updating specific treatment
  }) async {
    try {
      ...
      if (userTreatmentId != null) {
        ...
        // Create a new document every time when no specific ID is provided
...
// New method to get a specific user treatment by ID
  Future<Map<String, dynamic>?> getUserTreatmentById(
      String userTreatmentId) async {
    try {
      final userId = _getCurrentUserId();
...
// New method to get user's latest treatment for a specific treatment type
  Future<Map<String, dynamic>?> getLatestUserTreatment(
